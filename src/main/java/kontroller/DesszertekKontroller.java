...
public class DesszertekKontroller {
    ...
    private Logger logger = LoggerFactory.getLogger(EloetelekKontroller.class);
    ...
    private void tetelfelvetel(ActionEvent event) {
        Desszert desszert = dao.keresDesszert(((Button)(event.getSource())).getText());
        if(desszert == null)
            return;
        rendeles.hozzaadTetel(desszert);
        logger.info("Elotel hozzaadva a rendeleshez!");
        refresh();
    }
