...
public class EloetelekKontroller {
    ...
    private void tetelfelvetel(ActionEvent event) {
        Eloetel eloetel = dao.keresEloetel(((Button)(event.getSource())).getText());
        if(eloetel == null)
            return;
        rendeles.hozzaadTetel(eloetel);
        logger.info("Etel hozzaadva a rendeleshez!");
        refresh();
    }
